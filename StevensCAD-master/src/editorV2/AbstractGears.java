...
public class AbstractGears {
    ...
    private static void gear(float inner_radius, float outer_radius, float width, int teeth, float tooth_depth) {
        ...
        for (i = 0; i <= teeth; i++) {
            angle = i * 2.0f * (float)PI / teeth;

            glVertex3f(r0 * (float)cos(angle), r0 * (float)sin(angle), width * 0.5f);
            glVertex3f(r1 * (float)cos(angle), r1 * (float)sin(angle), width * 0.5f);
            if (i < teeth) {
                glVertex3f(r0 * (float)cos(angle), r0 * (float)sin(angle), width * 0.5f);
                glVertex3f(r1 * (float)cos(angle + 3.0f * da), r1 * (float)sin(angle + 3.0f * da),
                    width * 0.5f);
            }
        }
        glEnd();
        ...
        for (i = 0; i < teeth; i++) {
            ...
            glVertex3f(r2 * (float)cos(angle + 2.0f * da), r2 * (float)sin(angle + 2.0f * da), width * 0.5f);
            glVertex3f(r1 * (float)cos(angle + 3.0f * da), r1 * (float)sin(angle + 3.0f * da), width * 0.5f);
        }
        glEnd();
        ...
        for (i = 0; i < teeth; i++) {
            ...
            float u = r2 * (float)cos(angle + da) - r1 * (float)cos(angle);
            float v = r2 * (float)sin(angle + da) - r1 * (float)sin(angle);

            float len = (float)sqrt(u * u + v * v);
